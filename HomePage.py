...
class Main_Page:
    def __init__(self, root, ls):
        ...
        self.title1 = Label(self.frame1, text='TVD HOMEPAGE', font=('Algerian', 25, 'bold'), bg='#ffffff').pack(side=TOP)

        # Creating two buttons.
        self.student_btn = Button(self.frame1, text='STUDENT PORTAL', bd=0, bg='#ffffff', image=self.photo_student,
                                   compound=TOP, command=self.student) \
            .place(width=ls[0] // 9, height=ls[1] // 3, x=(ls[0]//9)*3-ls[0]//9*2, y=(ls[1] // 3)-ls[1]//4)

        self.employee_btn = Button(self.frame1, text="FACULTY PORTAL", bd=0, bg='#ffffff', image=self.photo_employee,
