...
class Registration:
    def __init__(self, root, ls, frame_old2):
        ...
        # Labeling the title
        title = Label(self.frame, text='TVD UNIVERSITY', font=('Algerian', 25, 'bold'),
        ...
        m_title = Label(Manage_Frame, text="MANAGE STUDENT", bg="lightpink", fg="blue",
        ...
        lbl_Location = Label(Manage_Frame, text="LOCATION",bg="LightBlue", fg="RED",
                            font=("times new roman", 20, "bold"))
        lbl_Location.grid(row=7, column=0, pady=10, padx=20, sticky="w")

        self.txt_Location = Text(Manage_Frame, width=30, height=3, font=("", 10))
        self.txt_Location.grid(row=7, column=1, pady=10, padx=20, sticky="w")
        ...
        Adtn = Button(btn_frame, text="ADD",bg='black',fg='white', width=10, command=self.add_data).grid(row=0, column=0, padx=10, pady=10)
        ...
        self.Student_table = ttk.Treeview(Table_Frame,
                                     columns=("Roll", "Name", "Email", "Gender", "Phone", "DOB", "Location"),
        ...
        self.Student_table.column("Location", width=200)
    ...
    def add_data(self):
        conn =mysql.connector.connect(host="localhost",user="root",passwd='',database="employee")
        c = conn.cursor()
        c.execute("INSERT INTO student_data VALUES (%s,%s,%s,%s,%s,%s,%s)",(self.Roll_No_var.get(),
                                                                 self.Name_var.get(),
                                                                 self.Email_var.get(),
                                                                 self.Gender_var.get(),
                                                                 self.Phone_var.get(),
        ...
        c.execute("INSERT INTO student_fees VALUES (%s,%s,%s)", (self.Roll_No_var.get(), '0', '0'))
        c.execute("INSERT INTO student_attendance VALUES (%s,%s,%s)", (self.Roll_No_var.get(), '0', '0'))
        conn.commit()
        self.fetch()
        self.clear()
        conn.close()
    ...
    def get_cursor(self, ev):
        ...
        self.txt_Location.insert(END, row[6])

    def update_data(self):
        conn =mysql.connector.connect(host="localhost",user="root",passwd='',database="employee")
        cur = conn.cursor()
        cur.execute("UPDATE student_data SET name='{}', email='{}', gender='{}', phone='{}', DOB='{}', Location='{}' where Roll_no= {}"
    ...
    def exiting(self):
        self.frame.destroy()
        self.root.title("TVD UNIVERSITY")
